...
export class Along extends RebarBase {
  ...
  buildPos(): this {
    ...
    bar.pos.sCol.dot(
      -t.col.h / 2 + as + fig.r,
      ...this.genDividedLine(0, fig.r).points.map((p) => p.y),
      t.col.h / 2 - as - fig.r
    );
    return this;
  }
  ...
  protected genDividedLine(x = 0, r = 0): Line {
    ...
    const as = this.rebars.as;
    const bar = this.rebars.column.along;
    return new Line(vec(x, -t.col.h / 2 + as + r), vec(x, t.col.h / 2 - as - r))
    ...
    if (space) {
      ...
      fig.push(
        fig
          ...
          .rebar(
            ...xs.map(
              (x) => new Line(vec(x, t.h - as), vec(x, -t.found.hn + as))
            )
          )
          .spec(bar, bar.singleCount)
          .leaderNote(vec(-t.col.h / 2 - fig.h, y), vec(1, 0))
          .generate()
      );
    }
  }
