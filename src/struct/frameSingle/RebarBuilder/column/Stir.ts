...
export class Stir extends RebarBase {
  ...
  buildSpec(): this {
    const t = this.struct;
    const bar = this.rebars.column.stir;
  ...
  protected buildHStir(): void {
    const t = this.struct;
    const as = this.rebars.as;
    const colBar = this.rebars.column;
    const count = colBar.along.singleCount;
    const h =
      (t.col.h - 2 * as - colBar.corner.diameter) / (count + 1) +
  ...
  protected buildVStir(): void {
    ...
    const h =
      (t.col.w - 2 * as - colBar.corner.diameter) / (count + 1) +
  ...
  protected genMulColYPos(): number[] {
    ...
    for (let i = 0; i < count; i++) {
      ...
      h -= l;
      res.push(...line.points.map((p) => p.y));
    }
    return res;
  }
  protected drawCross(): void {
    ...
    const x0 = -t.w / 2 + as;
    const x1 = -t.hsn / 2 - as;
    const left = ys.map((y) => new Line(vec(x0, y), vec(x1, y)));
    const x = this.rebars.column.cross.pos.cross.find(-t.hs / 2);
    const leftRebar = fig
      .planeRebar()
      .rebar(...left)
      .spec(bar, ys.length)
  ...
  protected drawAlong(): void {
    ...
    const x = this.rebars.column.along.pos.along.find(0);
    fig.push(
      fig
        .planeRebar()
        .rebar(...lines)
  ...
  protected drawSCol(): void {
    ...
    const pos = this.rebars.column.along.pos.sCol;
    fig.push(
      fig
        .planeRebar()
        .rebar(
  ...
  protected drawSColHStir(): void {
    ...
    const as = this.rebars.as;
    const fig = this.figures.sCol;
    const pos = this.rebars.column.cross.pos.sCol;
    const ys = new Line(
      vec(0, t.col.h / 2 - as - fig.r),
      vec(0, -t.col.h / 2 + as + fig.r)
    )
      .divideByCount(this.rebars.column.along.singleCount + 1)
      .removeBothPt()
      .points.map((p) => p.y);
    const w = t.col.w - 2 * as;
    const x = pos.head;
    ...
    if (ys.length >= 2) {
      const rebar = fig
        .planeRebar()
        .spec(this.hStirs[iStir++], Math.floor(ys.length / 2));
  ...
  protected drawSColVStir(): void {
    ...
    const pos = this.rebars.column.along.pos.sCol;
    const xs = new Line(
      vec(-t.col.w / 2 + as + fig.r, 0),
      vec(t.col.w / 2 - as - fig.r, 0)
    )
      .divideByCount(this.rebars.column.cross.singleCount + 1)
      .removeBothPt()
      .points.map((p) => p.x);
    ...
    if (xs.length >= 2) {
      const rebar = fig
        .planeRebar()
        .spec(this.vStirs[iStir++], Math.floor(xs.length / 2));
      ...
      const y = pos.head;
      fig.push(
        rebar.leaderNote(vec(t.col.w / 2 + fig.h, y), vec(1, 0)).generate()
      );
    }
    if (xs.length % 2 === 1) {
