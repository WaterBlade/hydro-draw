...
export class Corner extends RebarBase {
  buildSpec(): this {
    const t = this.struct;
    const bar = this.rebars.column.corner;
  ...
  protected drawCross(): void {
    ...
    const pos = this.rebars.column.stir.pos;
    if (space) {
      ...
      const left = fig
        .planeRebar()
        .rebar(
          new Polyline(-t.w / 2 + as, t.h - as)
            .lineBy(0, -t.h + as - t.found.hn + as)
            .lineBy(-500, 0),
          new Polyline(-t.hsn / 2 - as, t.h - as)
            .lineBy(0, -t.h + as - t.found.hn + as)
            .lineBy(500, 0)
        )
        .spec(bar, 2)
    ...
    if (space) {
      ...
      fig.push(
        fig
          .planeRebar()
          .rebar(
            new Polyline(-t.col.h / 2 + as, t.h - as)
              .lineBy(0, -t.h + as - t.found.hn + as)
              .lineBy(-300, 0),
            new Polyline(t.col.h / 2 - as, t.h - as)
              .lineBy(0, -t.h + as - t.found.hn + as)
  ...
  protected drawSCol(): void {
    ...
    const left = [
      fig
        ...
        .spec(bar)
        .parallelLeader(
          vec(-t.col.w / 2 - fig.h, t.col.h / 2 + fig.h),
          vec(1, 0)
        )
        .generate(),
      fig
