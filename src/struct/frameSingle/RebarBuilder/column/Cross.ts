...
export class Cross extends RebarBase {
  buildSpec(): this {
    const t = this.struct;
    const bar = this.rebars.column.cross;
    const form = RebarFormPreset.SShape(
      bar.diameter,
      500,
      t.h + t.found.hn - this.rebars.as * 2,
      300,
      7
    )
  ...
  buildPos(): this {
    ...
    bar.pos.sCol.dot(
      -t.col.w / 2 + as + r,
      ...this.genDividedLine(0, r).points.map((p) => p.x),
      t.col.w / 2 - as - r
    );
    return this;
  }
  ...
  protected genDividedLine(y = 0, r = 0): Line {
    const t = this.struct;
    const as = this.rebars.as;
    const bar = this.rebars.column.along;
    return new Line(vec(-t.col.w / 2 + as + r, y), vec(t.col.w / 2 - as - r, y))
  ...
  protected drawCross(): void {
    ...
    if (space) {
      ...
      const bar = this.rebars.column.cross;
      const as = this.rebars.as;
      const y = this.rebars.column.stir.pos.find(space.mid);
      const xs = this.genPos();
      const left = fig
        .planeRebar()
        .rebar(
          ...xs.map((x) => new Line(vec(x, t.h - as), vec(x, -t.found.hn + as)))
        )
  ...
  protected drawSCol(): void {
    ...
    fig.push(
      fig
        .linePointRebar()
        .line(this.genDividedLine(t.col.h / 2 - as - fig.r, fig.r))
        .spec(bar, bar.singleCount)
