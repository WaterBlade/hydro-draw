...
export class Stir extends RebarBase {
  ...
  buildSpec(): this {
    const t = this.struct;
    const bar = this.rebars.beam.stir;
  ...
  protected buildTendonSpec(): void {
    ...
    const preBar = this.rebars.beam.stir;
    const as = this.rebars.as;
  ...
  buildPos(): this {
    this.rebars.beam.stir.pos.dot(...this.genMulPos());
    return this;
  }
  protected genMulPos(): number[] {
    ...
    return new Line(vec(-t.hsn / 2 + d, 0), vec(t.hsn / 2 - d, 0))
      .divide(this.rebars.beam.stir.space)
      .points.map((p) => p.x);
  }
  buildFigure(): this {
    this.drawCross();
  ...
  protected drawCross(): void {
    ...
    const bar = this.rebars.beam.stir;
    const midBar = this.rebars.beam.mid;
    const as = this.rebars.as;
    const n = t.n;
    if (n > 0) {
  ...
  protected drawSBeam(): void {
    ...
    const bar = this.rebars.beam.stir;
    const as = this.rebars.as;
    const y = this.rebars.beam.mid.pos.sBeam.tail;
    fig.push(
      fig
        .planeRebar()
    ...
    const ys = this.rebars.beam.mid.pos.sBeam.dots.slice(1, -1);
    ...
    fig.push(
      rebar
        .cross(
          new Polyline(0, -t.beam.h / 2)
            .lineTo(0, y)
            .lineBy(t.beam.w / 2 + fig.h, 0)
        )
        .note()
        .generate()
    );
