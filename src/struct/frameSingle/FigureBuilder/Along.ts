  ...
  buildOutline(): this{
    ...
    fig.addOutline(
      ...
      new Polyline(-t.topBeam.w/2, t.h)
        ...
        .lineBy(0, t.topBeam.h+t.topBeam.ha)
        .dashedLine(),
      new Line(vec(-t.topBeam.w/2, t.h-t.topBeam.h), vec(t.topBeam.w/2, t.h-t.topBeam.h)).dashedLine()
    );
    ...
    for(let i = 1; i < t.n+1; i++){
      ...
      const y1 = y0 - h;
      fig.addOutline(
        new Polyline(x0, y0+ha).lineBy(w, 0).lineBy(0, -h-2*ha).lineBy(-w, 0).lineBy(0, h+2*ha).dashedLine(),
    ...
    fig.addOutline(
      ...
      new Line(vec(-2*t.col.h, -t.found.h), vec(2*t.col.h, -t.found.h)).greyLine(),
    );
