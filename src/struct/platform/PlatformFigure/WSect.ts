...
export class WSect extends Figure{
  initFigure(): void{
  ...
  protected buildOutline(t: PlatformStruct): void{
    const fig = this.fig;
    fig.addOutline(
      new Polyline(-t.w/2, -t.h/2).lineBy(0, t.h).lineBy(t.w, 0).lineBy(0, -t.h).greyLine(),
    );
    const xs = t.wBottomDivide().slice(1, -1);
    fig.addOutline(
      new Line(vec(-t.w/2, -t.h/2), vec(xs[0], -t.h/2)).greyLine(),
      new Line(vec(t.w/2, -t.h/2), vec(xs[xs.length-1], -t.h/2)).greyLine(),
    )
  ...
  protected buildLTop(t: PlatformStruct, rebars: PlatformRebar): void{
    ...
    fig.push(
      fig.linePointRebar()
        .line(new Line(vec(-t.w/2+as+fig.r, y), vec(t.w/2-as-fig.r, y)).divide(bar.space))
        .offset(2*fig.h)
  ...
  protected buildLBot(t: PlatformStruct, rebars: PlatformRebar): void{
    ...
    const line = new Line(vec(-t.w/2+as+fig.r, y), vec(t.w/2-as-fig.r, y)).divide(bar.space);
    const ptNotInside = [];
    const xs = t.wBottomDivide();
    ...
    fig.push(
      fig.linePointRebar()
        ...
        .spec(bar.lBot, bar.lBot.count, bar.space)
        .onlineNote()
        .generate()
    );
  }
  protected buildWBot(t: PlatformStruct, rebars: PlatformRebar): void{
    ...
    fig.push(
      fig.planeRebar()
        ...
        .leaderNote(vec(-t.w/2+fig.h*2, -t.h/2-4*fig.h), vec(0, 1), vec(-1, 0))
        .generate()
    );
  ...
  protected buildWTop(t: PlatformStruct, rebars: PlatformRebar): void{
    ...
    fig.push(
      fig.planeRebar()
        .rebar(new Polyline(-t.w/2+as, -t.h/2+as).lineBy(0, t.h-2*as).lineBy(t.w-2*as, 0).lineBy(0, -t.h+2*as))
