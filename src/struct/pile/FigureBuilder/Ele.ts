...
export class Ele extends PileFigureContext{
  initFigure(): this {
  ...
  protected drawMain(): void {
    ...
    const fig = this.figures.ele;
    const xs = this.pos.main();
    const n = xs.length;
    const da = (2 * t.topAngle) / (n - 1);
    const h0 =
      bar.diameter * this.rebars.anchorFactor * Math.cos(toRadian(t.topAngle));
    let a = -t.topAngle;
    ...
    rebar
      .spec(bar)
      .leaderNote(
        vec(-t.d / 2 - 2 * fig.h, -5.5 * this.rebars.stir.denseSpace),
        vec(1, 0)
      );
  ...
  protected drawTopStir(): void {
    ...
    const fig = this.figures.ele;
    const lines = this.shape.stirTop();
    fig.push(
      fig
        .planeRebar()
        .rebar(...lines)
