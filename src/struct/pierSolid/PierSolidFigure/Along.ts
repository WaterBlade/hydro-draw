...
export class PierSolidAlong extends Figure{
  initFigure(): void{
  ...
  protected lMain(t: PierSolidStruct, rebars: PierSolidRebar): void{
    ...
    const plLeft = new Polyline(-t.w.val/2 + as, t.h.val + t.topBeam.h.val - as)
    ...
    fig.push(
      fig.planeRebar()
        ...
        .leaderNote(vec(-t.w.val/2-2*fig.h, t.h.val-3*fig.h), vec(1, 0))
        .generate()
    );
  }
  protected wMain(t: PierSolidStruct, rebars: PierSolidRebar): void{
    ...
    fig.push(
      fig.planeRebar()
        .rebar(
          ...pos.map(x => new Line(vec(x, -t.found.h.val+as), vec(x, t.h.val + t.topBeam.h.val - as)))
        )
        .spec(bar.spec, pos.length, bar.space)
        .leaderNote(vec(-t.w.val/2-2*fig.h, t.h.val-6*fig.h), vec(1, 0))
        .generate()
    );
  }
}