...
class BeamBot extends CountRebar{
  build(t: BeamStruct, name: string): void{
    ...
    this.spec.setCount(this.singleCount * t.n).setForm(form).setId(this.container.id).setName(name);
...
class BeamMid extends CountRebar{
  build(t: BeamStruct, name: string): void{
    ...
    this.spec.setCount(this.singleCount * t.n * 2).setForm(form).setId(this.container.id).setName(name);
    this.container.record(this.spec);
  }
}
...
class Stir extends SpaceRebar{
  build(t: BeamStruct, name: string): void{
    ...
    const count = divideBySpace(-t.ln/2, t.ln/2, this.space).slice(1, -1).length * t.n;
    this.spec
      .setCount(count)
      .setForm(form)
