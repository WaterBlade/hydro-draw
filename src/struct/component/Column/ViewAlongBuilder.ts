...
export class ColumnViewAlongBuilder extends ColumnContext{
  protected yLeader = 0;
  ...
  generate(fig: Figure): CompositeItem{
    ...
    this.dot = divideByCount(-t.h/2+as, t.h/2-as, this.rebars.along.singleCount + 1);
  ...
  protected along(fig: Figure, comp: CompositeItem): void{
    this.yLeader -= 3*fig.h;
    const t = this.struct;
