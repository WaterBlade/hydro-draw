...
export class DXFPaper implements Paper {
  ...
  protected getLayer(lineType: LineType): Layer {
    ...
    switch (lineType) {
      ...
      case LineType.Thick:
        return layer.thick;
      case LineType.Thicker:
        return layer.thicker;
  ...
  protected getTextAlignCode(
    textAlign: TextAlign
  ): { code72: number; code73: number } {
    let code72 = 0;
    let code73 = 0;
    switch (textAlign) {
  ...
  visitArc(arc: PaperArc): void {
    this.entities.push(
      new DXFArc(arc, this.handleGen.handle, this.getLayer(arc.lineType))
    );
  }
  visitArrow(arrow: PaperArrow): void {
    this.entities.push(
      new DXFArrow(arrow, this.handleGen.handle, this.getLayer(arrow.lineType))
    );
  }
  ...
  visitLine(line: PaperLine): void {
    this.entities.push(
      new DXFLine(line, this.handleGen.handle, this.getLayer(line.lineType))
    );
  }
