...
export default function runFrameSingleDemoBaiyandong(): void{

  const paper = new DXFPaper();
  const layout = new HLayoutBuilder(100);

  for(let i = 2000; i <= 14000; i += 500){
    layout.push(...genFrame(i));
  }

  paper.push(layout.generate());

  fs.writeFile('左二白眼洞排架标准钢筋图.dxf', paper.generate(), ()=>{
    console.log('完成白眼洞排架标准钢筋图');
  })
...
function genFrame(height: number): DrawItem[]{
  ...
  draw.design = '技施';
  draw.drawingTitle = `白眼洞H=${height.toFixed(0)}单排架钢筋图`;
